    ...
    : Node("publisher_node"), file_index_(0)
{
  ...
  init_file_path(); // the paths of origin data

  create_publishers_data_file_names();

  auto callback = [this]()
  {
    return this->on_timer_callback();
  };
...
void KittiPublishersNode::on_timer_callback()
{
  ...
  convert_pcl_to_pointcloud2(point_cloud2_msg); // the ros2 pointcloud msg read from data file
  ...
  // Concatenating image file paths
...
{
  // 创建一个 pcl::PointCloud<pcl::PointXYZI> 类型的对象 cloud
  ...
  // 将 pcl::PointCloud 转换为 sensor_msgs::msg::PointCloud2 类型的消息
...
void KittiPublishersNode::convert_image_to_msg(sensor_msgs::msg::Image &msg, const std::string path)
{
  ...
  // std::vector的内部存储是连续的，元素在内存中是紧密排列的。
  // 通过获取指向第一个元素的指针，可以访问整个内部存储区域。
  // 这种情况下，&msg.data[0]返回一个uint8_t*指针，指向msg.data的第一个元素。
